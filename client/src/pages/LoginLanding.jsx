...
const LoginLanding = () => {
  ...
  const portalOptions = [
    ...
    },
    {
      ...
      description: "View your profile, track attendance, request time off, and access paylips.",
      icon: UserIcon
    }
  ]
      ...
      <div className="w-full max-w-md animate-fade-in relative z-10">
        ...
        <div className="space-y-4">
          {portalOptions.map((portal)=>(
            <Link key={portal.to} to={portal.to} className="group block bg-slate-50 border border-slate-200 rounded-lg p-5 sm:p-6 transition-all duration-500 hover:border-indigo-400 hover:bg-indigo-50">
              <div className="relative z-10 flex items-center justify-between gap-4 sm:gap-5">
                ...
                <ArrowRightIcon className="w-4 h-4 text-slate-400 group-hover:text-indigo-600 group-hover:translate-x-1 transition-all duration-500"/>
              </div>
            </Link>
          ))}
        </div>
        ...
        <div className="mt-12 text-center md:text-left text-sm text-slate-400">
          <p>© {new Date().getFullYear()} Lian Wiebe. All rights reserved.</p>
        </div>
